...
void GranadaRoja::Shoot(std::vector<std::shared_ptr<Projectile>>& projectiles, uint32_t& entity_id, float x_pos, float y_pos, float angle) {
    ...
    nivel_carga = 0;
    cargando = false;
    std::shared_ptr<Grenade> grenade = std::make_shared<Grenade>(world, ProjectileType::RED_GRENADE, x_pos, y_pos, angle, power, dmg, radius, fragments, GRENADE_BASE_RESTITUTION, seconds);
